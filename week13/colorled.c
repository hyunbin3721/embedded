...
int pwmActiveAll(void)  //PWM 채널 활설화
{
    int fd = 0;
...
int pwmInactiveAll(void)  //PWM 끄기
{
    int fd = 0;
...
int pwmSetDuty(int dutyCycle, int pwmIndex)  //PWM duty 값 지정
{
    int fd = 0;
...
int pwmSetPeriod(int Period, int pwmIndex)  //PWM period 값 지정
{
    int fd = 0;
...
int pwmSetPercent(int percent, int ledColor)   //PWM %값 지정
{
    
    if((percent <0) || (percent > 100))
    {
        printf ("Wrong percent: %d plz insert 0 ~ 100 \r\n", percent);
        return 0;
    }
